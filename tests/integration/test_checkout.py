...
def _payload(key: str, offer_id: str = "SEN-MOM4-BLK") -> dict[str, object]:
    return {
        "offer_id": offer_id,
...
def test_prepare_is_persistent_idempotent_and_server_authoritative() -> None:
    ...
    assert first.data.pricing.total_minor == 1_274_000_00
...
def test_same_key_with_different_content_conflicts() -> None:
    ...
    conflict = service.prepare(_payload("reused-key", offer_id="SEN-MOM4-WHT"))
...
def test_ready_for_payment_lifecycle_is_persisted_and_event_idempotent() -> None:
    ...
    service = CheckoutService(authenticate_lab_session(token))
    prepared = service.prepare(_payload("ready-create", offer_id="APL-APP3"))
    assert isinstance(prepared, Success)
    checkout_id = prepared.data.id
    ...
    expiring = service.prepare(_payload("ready-expiring", offer_id="SON-XM6-BLK"))
